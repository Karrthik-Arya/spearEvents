...
import { type Group, type Event } from '../../types/events'
...
type CreateGroupScreenProps = AppStackScreenProps<'CreateGroupScreen'>
...
const CreateGroupScreen: FC<CreateGroupScreenProps> = ({ navigation }) => {
  const formFactory = createFormFactory<Group>({
    defaultValues: {
      title: '',
      schedule: '',
      tags: []
    ...
    <Box flex={1} bgColor={'white'} alignItems={'center'}>
        ...
        {(field) =>
            <FormControl w="80%">
                ...
                <Input value={field.state.value} onChangeText={field.handleChange} outlineColor={'black'} placeholder="Whats your gang called?" variant={'rounded'} />
        ...
        <FormControl w="80%" >
                <FormControl.Label _text={{ color: 'black' }}>{field.name.charAt(0).toUpperCase() + field.name.slice(1)}</FormControl.Label>
                <Input value={field.state.value} onChangeText={field.handleChange} outlineColor={'black'} placeholder="After Party, Ice Breaking ..." variant={'rounded'} />
                <FormControl.ErrorMessage leftIcon={<WarningOutlineIcon size="xs" />}>
                Bleh!! Pls Enter an appropriate response
                </FormControl.ErrorMessage>
        </FormControl>}
